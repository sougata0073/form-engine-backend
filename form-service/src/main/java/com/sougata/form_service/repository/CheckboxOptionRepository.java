...
public interface CheckboxOptionRepository extends JpaRepository<CheckboxOption, Long> {
    ...
    @Modifying
    @Transactional
    @Query("delete from CheckboxOption co where co.checkbox.question.form.id = :formId and co.checkbox.questionId = :checkboxId")
    void deleteAllByFormIdAndCheckboxId(UUID formId, long checkboxId);
