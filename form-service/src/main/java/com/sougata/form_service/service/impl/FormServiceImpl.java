...
import com.sougata.form_service.exception.RequiredQuestionResponseNotFoundException;
...
public class FormServiceImpl implements FormService {
    ...
    @Override
    public FormAddUpdateResDto createForm(FormAddUpdateReqDto dto) {
        Form newForm = new Form();

        newForm.setUserId(dto.userId());
        ...
        return FormAddUpdateResDto.create(savedForm);
    }

    @Override
    public FormAddUpdateResDto updateForm(UUID formId, FormAddUpdateReqDto dto) {
        Form f = getFormById(formId);

        f.setUserId(dto.userId());
    ...
    @Override
    public FormResponseDto viewForm(UUID id) {
        Form f = getFormById(id);
    ...
    public SuccessMessageDto validateResponse(UUID formId, ResponseValidationRequestDto dto) {
        ...
        questionRepositoryFactory.getAll().forEach(qRepo -> {
            requiredQuestionIds.addAll(
                    qRepo.findIdByFormIdAndRequired(formId, true)
            );
        });
        ...
        dto.responses().forEach(vReq -> {
            ...
            if (!isValid) {
                throw new ResponseValidationException(
                        String.format(ValidationMessages.INVALID_RESPONSE, vReq.getQuestionType(), vReq.getValidationId())
                );
            }
        });
